...
export class InMemoryDB implements IDataStore {
  ...
  async createUser(user: IUser): Promise<void | string> {
  ...
  getAllProblems(tags?: ITag[], userId?: string): Promise<IProblem[]> {
    ...
    if (tags && tags.length > 0) {
      filteredProblems = filteredProblems.filter(problem => {
        ...
        return tags.some(tag => problemTags.some(problemTag => problemTag.id === tag.id))
      })
    }

    if (userId) {
      filteredProblems = filteredProblems.filter(problem => problem.userId === userId)
    }
  ...
  async getProblemsByTags(tags: ITag[]): Promise<IProblem[]> {
    const problemsByTags = this.problems.filter(problem => {
      if (problem.tags) {
        return tags.every(tag => problem.tags!.some(problemTag => problemTag.id === tag.id))
      }
      return false
    })
  ...
  async getProblemsByUser(userId: string): Promise<IProblem[]> {
    const problemsByUser = this.problems.filter(problem => problem.userId === userId)
    return Promise.resolve(problemsByUser)
  }
  async updateProblem(problem: Partial<IProblem>, userId?: string | undefined): Promise<IProblem> {
  ...
  public async getSolution(id: string | ISolution): Promise<ISolution | null | undefined> {
  ...
  public async updateSolution(solution: Partial<ISolution>): Promise<ISolution> {
  ...
  getAllReacts(contentType: 'problem' | 'solution', contentId: string): Promise<IReact[]> {
    return Promise.resolve(
      this.reacts.filter(r => r.contentType === contentType && r.contentId === contentId)
    )
  }
