...
import { ForbiddenException } from '@web/rest/exceptions';
import { RequestUtils } from '@web/rest/infrastucture/request.utils';
import { ResponseUtils } from '@web/rest/infrastructure/response.utils';
...
class AuthenticationMiddleware implements MiddlewareMethods {
  ...
  private ensureUserHasPrivileges(
    ...
    const userRoles =
      validatedSessionResponse.accessToken?.roles.map(role =>
    ...
    const userHasPrivileges =
      ...
      allowedRoles.some((role: UserRoles) =>
